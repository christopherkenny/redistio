...
integrityServer <- function(id, shp, redistio_curr_plan, adj, split_cols) {
  shiny::moduleServer(id, function(input, output, session) {
    output$integrity <- gt::render_gt({
      if (!anyNA(redistio_curr_plan$pl)) {
        int_l <- list(
          rict::rict_population(
            shp,
            plan = redistio_curr_plan$pl,
            as_gt = FALSE
          ),
          rict::rict_contiguity(
            shp,
            plan = redistio_curr_plan$pl,
            adj = adj,
            as_gt = FALSE
          ),
          rict::rict_compactness(
            shp,
            plan = redistio_curr_plan$pl,
            as_gt = FALSE
          ),
          rict::rict_splits(
            shp,
            plan = redistio_curr_plan$pl,
            admin = split_cols$admin,
            subadmin = split_cols$subadmin,
            multi = split_cols$multi,
            total = split_cols$total,
            as_gt = FALSE
          )
        )
      } else {
      ...
      int_l |>
        ...
        gt::tab_spanner(
          label = 'Multi Splits',
          columns = dplyr::starts_with('multi_')
        ) |>
        gt::tab_spanner(
          label = 'Total Splits',
          columns = dplyr::starts_with('total_')
        ) |>
        gt::cols_label(
        ...
        gt::data_color(
          ...
          fn = function(x) {
            scales::col_numeric(
              palette = palette_deviation,
              domain = c(0, max(abs(x), na.rm = TRUE))
            )(abs(x))
          }
        ) |>
        gt::data_color(
