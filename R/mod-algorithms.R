...
#'
#' @param id module id
#' @param ... args to pass to layout_columns
#'
#' @return shiny UI
#' @noRd
algorithmsUI <- function(id, ...) {
  ns <- shiny::NS(id)
  bslib::nav_panel(
    title = 'algorithms',
    bslib::layout_columns(
      ...
      bslib::card( # selector
        shiny::selectizeInput(
          ...
          label = paste0('Districts to redraw (up to ', min(2, 5), ')'),
          choices = seq_len(5),
...
#' @return shiny server
#' @noRd
algorithmsServer <- function(id, shp, shp_in, curr_plan, ndists, pal,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$alg_run, {
      base_id <- 1e6
      alg_pal <- as.character(pal())
      output$alg_map <- mapgl::renderMaplibre({
        map_sub(shp |>
          dplyr::mutate(redistio_plan = curr_plan$pl) |>
      ...
      mapgl::maplibre_proxy(session$ns('alg_map')) |>
        mapgl::set_paint_property(
          ...
          value = discrete_palette(pal(), redistio_alg_plan$plans[, input$alg_summary_rows_selected],
                                   column = 'redistio_sub_id', base = 1e6)
        )
    })
    ...
    shiny::observeEvent(input$alg_accept, {
      ...
      idx <- which(curr_plan$pl %in% input$alg_district)
      curr_plan$pl[idx] <- pl

      undo_l(undo_log(undo_l(), curr_plan$pl))

      mapgl::maplibre_proxy('map') |>
        update_shape_style(
          fill_column, pal(), curr_plan$pl, shp,
          fill_opacity, precinct_border
        )
      ...
      new_tb_pop <- val()
      new_tb_pop$Population <- distr_pop(shp[[pop_col]], total = tot_pop, plan = curr_plan$pl, ndists = ndists)
